...
jmods version:
    rm -rf build/jmods
    mkdir -p build/jmods/src

    echo 'module dev.mccue.just {}' > build/jmods/src/module-info.java
    javac -d build/jmods/classes build/jmods/src/module-info.java

    mkdir -p build/jmods/built
    jmod create \
        --cmds build/repackaged/just-{{version}}-x86_64-apple-darwin/bin \
        --man-pages build/repackaged/just-{{version}}-x86_64-apple-darwin/bin \
        --class-path build/jmods/classes \
        build/jmods/built/just.jmod

    jresolve --output-directory build/jmods/built https://github.com/bowbahdoe/jresolve-cli/releases/download/v2024.05.10/jresolve.jar

link:
    rm -rf build/jre
    jlink \
        --module-path build/jmods/built \
        --add-modules java.se,dev.mccue.just,dev.mccue.jresolve.cli.uber \
        --output build/jre